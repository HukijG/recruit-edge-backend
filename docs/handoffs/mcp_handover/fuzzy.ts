...
export function tokens(s: string | undefined | null): string[] {
  return normalize(s)
    .split(/[\s,._\-()/\\|&]+/)
    .filter(Boolean);
}
...
const WORD_ALIASES_MAP: Record<string, string> = (() => {
 ...
 */
export function canonicalizeJobPhrase(s: string | undefined | null): string {
  if (!s) return "";
...
export function prepareTarget(s: string | undefined | null): PreparedTarget {
  const n = normalize(s);
...
export function scoreAny(
  query: string,
  targets: Array<string | PreparedTarget | undefined>,
): number {
  let best = 0;
...
export function recencyBoost(record: RecencyInput, now: Date = new Date()): number {
