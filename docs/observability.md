...
- **`trace-link`** — `src/lib/trace-link.js` (main + sync). `makeAsyncCallbackUrl(baseUrl, extraParams, options?)` (kickoff side) appends an `_otel_trace` query param containing the active trace's `traceparent`-shaped fingerprint. `readInboundTraceLink(request)` (receiving side) parses that param back into a `SpanContext` you can attach via `addLink({ context })` on the receiving span. Used for async webhook-callback patterns — the canonical site is Apollo enrichment kickoff + completion webhook. **Multi-hop chains:** pass `options.traceId` to pin the callback to the ORIGINATING flow's trace instead of the current hop's. The Apollo waterfall re-run loop uses this — `handleApolloWebhook` reads the inbound `_otel_trace` and threads it through `applyApolloEnrichment` into each re-run's callback URL, so every async re-run pass links back to the single originating extension-add flow rather than chaining hop-to-hop. The enrichment decision is also recorded as `apollo.*` span attributes (`apollo.region`, `apollo.stored`, `apollo.best_source`, `apollo.rerun`, `apollo.rerun_count`, `apollo.write_failed`, …) — kept by the postProcessor (not in `NOISE_KEYS`).
