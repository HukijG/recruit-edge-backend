...
The candidate/job resolvers run in two phases. **Phase 1** scores against the thin in-memory snapshot (`candidates_v2` / `jobs_v2`) using `scoreString` (prefix-exact, word-boundary substring, per-token Levenshtein on tokens ≥ 4 chars, extension-word penalty, equal-length bonus). **Phase 2** fans out to live RF (`/candidate/get` or `/job?job_id=…`) for the Phase 1 top-K to read mutable fields — `is_open` for jobs, `jobs[].stage_moved` (filtered to non-Sourced / non-Disqualified stages) for candidates — that intentionally aren't cached.

**Pool widening on flat-score ties.** Phase 1's `scoreString` produces identical 0.85 for any single-token name match against a two-token target (prefix coverage 1.0 / denominator 2 = 0.5 regardless of the last name) — so every "Jerry X" candidate ties exactly. Without intervention the deterministic cache iteration order picks which 5 reach Phase 2, and a real target at position 6+ never gets a stage-recency boost. The `widenTiedPool` helper extends the Phase 1 slice to include every candidate sharing the cutoff score, capped at `PHASE1_WIDE_CAP = 25` so a pathological query (e.g. typing a hyper-common first name) can't blow out the fan-out cost. Live-rerank concurrency stays at 5 inside `pMapLimit`, so 25 candidates ≈ 5 batches ≈ 750 ms worst-case.

**Long-tail truncation in disambiguation envelopes.** Phase 2's output goes through `truncateLongTail` which drops options scoring below `top * 0.85` (proportional, scales with Phase 2 boosts that can push scores past 1.0). A disambiguation envelope listing "Sales Engineer @ 0.87 / SEM @ 0.70 / SSE @ 0.63" wastes prompt tokens on the 0.70/0.63 entries — they're clearly not real competitors. The cutoff keeps the genuine alternatives (e.g. Sales Engineer @ 0.87 + Sales Engineer - US @ 0.80) while shaking off the long-tail.

Phase 2 fires whenever Phase 1 can't auto-resolve confidently. Auto-resolve requires `top.score >= 0.92` AND `top.score - second.score >= 0.08`. Anything else fans out the widened top-K via `pMapLimit` concurrency 5 (~150–300 ms for 5 candidates; ~750 ms worst-case for a widened pool of 25). Per-id fan-out failures degrade gracefully — the row keeps its Phase 1 score with a `_phase2: 'fetch_failed'` marker, so a transient RF blip doesn't drop a real match.

**`include_closed` flag.** `rf_job_pipeline` and `rf_job_candidates_filter` accept `include_closed: true` to bypass the Phase 2 closed-job filter. Default false — closed jobs are typically reachable via explicit numeric id, but the explicit override matches the `include_disqualified` pattern from `rf_candidate_search`.
