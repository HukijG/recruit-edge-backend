/**
 * Read-only enumeration of USERS_DB.users from sync-worker.
 *
 * USERS_DB is owned by the main worker (per CLAUDE.md "USERS_DB ... is owned
 * by the main worker"). Sync-worker binds it read-only here so cron tail-sync
...
export async function listConsultants(env) {
  if (!env?.USERS_DB) throw new Error('USERS_DB binding missing on sync-worker');
