...
/**
 * Module-id sentinel asserted by `mcp-remote/test/access-auth.spec.ts` to guard
 * against vite's relative-path resolver silently falling back to the main
 * worker's `src/access-auth.js` (one directory up from `mcp-remote/`) if this
 ...
 */
export const _MODULE_ID = "mcp-remote/access-auth";
