...
export function registerTools(server: McpServer, ctx: RequestCtx) {
  ...
  server.registerTool(
    "rf_candidate_search",
    {
      ...
      description: [
        ...
        "  • `added_after / added_before` — ISO dates (candidate creation time)",
        ...
        "",
        "Ordering: pure fuzzy → score-ranked (recency-boosted). Mutable filter + query → score-ranked over RF intersection. Mutable filter alone → RF default order.",
        "",
        ...
        "",
        "Response shape (success): `{ok: true, count, matches: [...]}`. Warnings: `warning: 'filter_unverified'` means RF was unreachable — results are tier-1 only, mutable filters were not applied. `warning: 'custom_field_map_unavailable'` means the RF custom-field schema fetch failed; `technology`/`segment`/`role` filters were dropped. `warning: 'rate_limited'` envelope (`ok:false, kind:'rate_limited', retry_after_ms`) at HTTP 200 on RF 429.",
        "",
      ...
      inputSchema: {
        ...
        added_after: z.string().optional().describe("ISO date — candidate creation (added_on)."),
        added_before: z.string().optional().describe("ISO date — candidate creation (added_on)."),
  ...
  server.registerTool(
    "rf_candidate_get",
    {
      ...
      description: [
        ...
        "",
        "Response shape: `{ok: true, candidate: {...}}` on success; `{ok: false, kind: 'no_candidate'|'rf_unavailable'|'rate_limited'|..., recoverable, error}` at HTTP 200 on RF failure or when no candidate matches; `{needs_disambiguation, kind:'candidate', options}` when fuzzy is ambiguous. All non-success / non-disambiguation paths return HTTP 200 with a `kind` discriminator — there is no HTTP 404 path.",
        "",
  ...
  server.registerTool(
    "rf_candidate_move_stage",
    {
      ...
      description: [
        ...
        "",
        "Recoverable failures come back at HTTP 200 as `{ok: false, kind, error, ...recovery_hints}` — use the hints to clarify with the user; don't crash. Kinds: `'no_candidate'` (candidate name/id doesn't resolve), `'not_on_job'` (candidate exists but isn't on the requested job), `'no_stage'` (stage id not on this job's pipeline), `'rate_limited'` / `'rf_unavailable'` (RF API trouble).",
        "",
  ...
  server.registerTool(
    "rf_candidate_log_interview",
    {
      ...
      description: [
        ...
        "",
        "Recoverable failures come back at HTTP 200 as `{ok:false, kind, error}` — apologise, clarify with the user, retry. Kinds: `'no_candidate'` (candidate doesn't resolve), `'rate_limited'` / `'rf_unavailable'` (RF API trouble). Ambiguous candidate → `{needs_disambiguation, kind:'candidate', options}`.",
        "",
        "Attribution is always the consultant whose Access JWT signed this MCP session — there is no override field.",
      ].join("\n"),
      inputSchema: {
  ...
  server.registerTool(
    "rf_job_candidates_filter",
    {
      ...
      description: [
        ...
        "",
        "Inputs: job (id or fuzzy ref), optional stage (single stage name; null = all stages), optional include_disqualified (default false), optional fields[] (defaults to thin: id, name, linkedin_profile), optional limit. Returns flat `{ok: true, job, total, matched: [...], truncated?, hydration_errors?: [{id, reason, status?}]}`. `hydration_errors` lists per-id failures when the expanded-field path can't reach RF for a subset of candidates.",
        "",
        "Field-vs-hydration trade-off: `current_title` / `current_organization` are served from the v2 cache's **at-first-sight snapshot** (never updated). Requesting them stays on the fast thin path. For LIVE values, pair with a richer field like `primary_email` to force the expanded `/candidate/get` fan-out, or call `rf_candidate_get` per id.",
        "",
  ...
  server.registerTool(
    "rf_job_pipeline",
    {
      ...
      description: [
        ...
        "",
        "Response shape: `{ok: true, job, stage_breakdown: [{stage_name, count}, ...], stages: {<stage_name>: [<candidates>]}, hydration_errors?: [{id, reason, status?}]}`. `stage_breakdown` is the ordered funnel (canonical pipeline order, restricted to requested window, includes empty buckets so you see funnel shape). `stages` is keyed by stage name for O(1) bucket access — iterate via `stage_breakdown` for order. `hydration_errors` lists per-id failures when the expanded-field path can't reach RF for a subset of candidates (partial result; never throws). Per-candidate defaults: `{id, name, linkedin_profile}` (linkedin is a full URL); pass `fields` to extend additively.",
        "",
        "Field-vs-hydration trade-off: `current_title` / `current_organization` are served from the v2 cache's **at-first-sight snapshot** (never updated). Requesting them stays on the fast thin path. For LIVE title / organization, pair with a richer field like `primary_email` to force the expanded `/candidate/get` fan-out, or call `rf_candidate_get` per id.",
        "",
  ...
  server.registerTool(
    "rf_cache_status",
    {
      ...
      description: [
        "Returns counts and last-sync state for the server-side D1 cache. Does NOT trigger a refresh.",
        "",
        "Response shape: `{ok: true, candidates_count, jobs_count, candidates_v2_count, jobs_v2_count, calls_count, last_tail_sync_at, last_tail_sync_count, minutes_since_last_sync, last_full_rebuild_at, last_candidates_added_cursor, in_flight}`. The `*_v2_count` fields are the authoritative thin-cache counts; legacy `candidates_count` / `jobs_count` remain during the dual-write window and surface as `null` after cutover step 6 drops the legacy tables. `last_candidates_added_cursor` is the cron's progress through RF candidates by `added_on` (ISO timestamp).",
        "",
        "The additive-only cron will run every 15 minutes once re-enabled (currently OFF — re-enable scheduled at cutover step 2/5). When OFF, `last_tail_sync_at` / `minutes_since_last_sync` may be stale; the v2 counts come from the most recent full rebuild.",
        "",
  ...
  server.registerTool(
    "rf_candidate_add_note",
    {
      ...
      description: [
        ...
        "",
        "Recoverable failures (`kind: 'no_candidate'` when no candidate match, `kind: 'rate_limited'` / `kind: 'rf_unavailable'` on RF API trouble, etc.) come back at HTTP 200 as `{ok:false, kind, error}` — apologise, clarify with the user, retry. 4xx / 5xx are loud install or transport errors.",
        "",
        "Attribution is always the consultant whose Access JWT signed this MCP session — there is no override field.",
      ].join("\n"),
      inputSchema: {
