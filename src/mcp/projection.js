 ...
 */
export function resolveFieldName(input, topLevelKeys, candidate) {
...
function pick(obj, tree) {
  ...
  if (Array.isArray(obj)) {
    ...
    if (starTree && typeof starTree === "object") {
      return obj.map((item) => pick(item, starTree));
    }
...
export function project(obj, paths) {
 ...
 */
export function resolveFields(inputs, sample, candidate) {
  const topKeys = Object.keys(sample);
  const paths = [];
  const notes = [];
  const errors = [];
