  ...
  server.registerTool(
    "rf_candidate_get",
    {
      ...
      description: [
        ...
        "",
        "Response shape: `{ok: true, candidate: {...}}` on success; `{ok: false, kind: 'rf_unavailable'|'rate_limited'|..., recoverable, error}` at HTTP 200 on RF failure; `{needs_disambiguation, kind:'candidate', options}` when fuzzy is ambiguous; HTTP 404 when no candidate matches.",
        "",
