...
let installBodyCapture: () => void;
let realFetchOriginal: typeof globalThis.fetch;
...
describe('body-capture', () => {
  beforeEach(async () => {
    ...
    globalThis.fetch = vi.fn(async (_input: any, _init?: any) => {
  ...
  it('stamps http.request.body and http.response.body attributes', async () => {
    ...
    const reqCall = calls.find(([k]: any) => k === 'http.request.body');
    const respCall = calls.find(([k]: any) => k === 'http.response.body');
