...
async function makeJwt(claims: any, aud = (env as any).ACCESS_AUD_MCP) {
  return new SignJWT(claims)
    .setProtectedHeader({ alg: 'RS256', kid: 'test-key' })
    .setIssuer((env as any).ACCESS_TEAM_DOMAIN)
    .setAudience(aud)
    .setIssuedAt()
...
describe('auth gate (Access JWT)', () => {
  it('returns 401 with no JWT header', async () => {
    const res = await worker.fetch(makeRequest({}), env as any, EMPTY_CTX);
  ...
  it('returns 200 ok on /health regardless of headers', async () => {
    const res = await worker.fetch(makeRequest({}, 'GET', '/health'), env as any, EMPTY_CTX);
    expect(res.status).toBe(200);
    expect(await res.text()).toBe('ok');
  });

  it('returns 404 for unknown paths', async () => {
    const res = await worker.fetch(makeRequest({}, 'GET', '/nope'), env as any, EMPTY_CTX);
