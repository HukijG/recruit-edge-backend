...
describe('makeLdResourceInjector — flow.name promotion to span.name', () => {
  it('overrides span.name with flow.name attribute when present', () => {
    ...
    const span = {
      ...
      resource: baseResource,
      attributes: { 'flow.name': 'WebhookDialpadExtensionCall' },
    };
    const [out] = injector([span]);
    expect(out.name).toBe('WebhookDialpadExtensionCall');
    // flow.name remains in attributes — needed for filter/groupBy queries
    expect(out.attributes['flow.name']).toBe('WebhookDialpadExtensionCall');
  });
