...
describe('FLOWS registry', () => {
  ...
  it('contains all known entry-point flow names', () => {
    const expected = [
      ...
      'WEBHOOK_RF', 'WEBHOOK_RF_MANUAL', 'WEBHOOK_RF_STAGE_MOVED',
      'WEBHOOK_DIALPAD_GENERAL',
      'WEBHOOK_DIALPAD_CALL', 'WEBHOOK_DIALPAD_EXT_CALL', 'COLD_CALL_CANCELLED_FINALIZE',
      'WEBHOOK_KRISP', 'WEBHOOK_CALENDAR', 'WEBHOOK_APOLLO_ENRICHMENT',
      'MCP_PROXY', 'HEALTH', 'TEST_COLD_CALL',
      'STATS_AGGREGATE_PULL', 'STATS_RECONCILE', 'STATS_BACKFILL',
    ];
