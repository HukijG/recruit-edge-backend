...
export function makeLdResourceInjector(sdkKey: string) {
  ...
  let cachedResource: any = null;

  return function postProcessor(spans: any[]): any[] {
...
function redactQueryParamsInString(value: string, key: string): string {
  try {
    if (key === 'url.full') {
...
function wrapSpanWithReplacements(span: any, replacements: Record<string, any>): any {
  ...
  try {
    const wrapper: any = Object.create(Object.getPrototypeOf(span));
  ...
  return new Proxy(span, {
    get(target: any, prop: string | symbol) {
      if (Object.prototype.hasOwnProperty.call(replacements, prop) &&
          replacements[prop as string] !== undefined && replacements[prop as string] !== null) {
        return replacements[prop as string];
      }
      return target[prop];
    },
  });
